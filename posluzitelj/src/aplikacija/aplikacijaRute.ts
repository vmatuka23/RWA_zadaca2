...
import AktivacijskiTokenDAO, { AktivacijskiToken } from "../zajednicko/dao/aktivacijskiTokenDAO.js";
...
export class AplikacijaRute {
    ...
    private atdao: AktivacijskiTokenDAO;
    private db: Baza;
    ...
    private async register(zahtjev: Request, odgovor: Response): Promise<void> {
        ...
        try {
            ...
            // Spremi token u bazu
            const aktivacijskiToken: AktivacijskiToken = {
            ...
            try {
                const previewUrl = await emailServis.posaljiAktivacijskiEmail(
                    korisnickoIme,
                    email,
                    aktivacijskiTokenVrijednost
                );
     ...
     * Query: { token: string }
     */
    private async aktivirajRacun(zahtjev: Request, odgovor: Response): Promise<void> {
        const token = zahtjev.query["token"] as string;

        if (!token) {
            // Ako nema tokena, vrati HTML stranicu s greškom
            odgovor.type("text/html");
            odgovor.send(this.generirajAktivacijskuStranicu(
                false, 
                "Nedostaje aktivacijski token. Provjerite link iz emaila."
            ));
            return;
        }
        ...
        try {
            ...
            if (!aktivacijskiToken) {
                ...
                odgovor.send(this.generirajAktivacijskuStranicu(
                    false, 
                    "Aktivacijski token nije pronađen. Možda je već iskorišten ili je neispravan."
                ));
                return;
            }
            ...
            if (aktivacijskiToken.iskoristen === 1) {
                ...
                odgovor.send(this.generirajAktivacijskuStranicu(
                    false, 
                    "Ovaj aktivacijski link je već iskorišten. Vaš račun bi trebao biti aktiviran."
                ));
                return;
            }
            ...
            if (sada > datumIsteka) {
                ...
                odgovor.send(this.generirajAktivacijskuStranicu(
                    false, 
                    "Aktivacijski link je istekao. Kontaktirajte administratora za novi link."
                ));
                return;
            }
            ...
            if (!korisnik) {
                ...
                odgovor.send(this.generirajAktivacijskuStranicu(
                    false, 
                    "Korisnički račun nije pronađen."
                ));
                return;
            }
            ...
            odgovor.send(this.generirajAktivacijskuStranicu(
                true, 
                `Vaš račun "${korisnik.korisnickoIme}" je uspješno aktiviran! Sada se možete prijaviti.`
            ));
            ...
            odgovor.send(this.generirajAktivacijskuStranicu(
                false, 
                "Došlo je do greške pri aktivaciji. Pokušajte ponovno ili kontaktirajte administratora."
            ));
        }
    }
     ...
     * @returns HTML string
     */
    private generirajAktivacijskuStranicu(uspjeh: boolean, poruka: string): string {
...
<body>
     ...
     * Body: { email: string }
     */
    private async ponovoPosaljiAktivaciju(zahtjev: Request, odgovor: Response): Promise<void> {
        odgovor.type("application/json");
        ...
        try {
            ...
            // Pošalji aktivacijski email
            await emailServis.posaljiAktivacijskiEmail(
    ...
    public pripremiRute(server: Application): void {
        ...
        server.post("/api/login", (req: Request, res: Response) => this.login(req, res));
        server.post("/api/register", (req: Request, res: Response) => this.register(req, res));
        server.post("/api/logout", (req: Request, res: Response) => this.logout(req, res));
        
        // ===== AKTIVACIJA RAČUNA =====
        server.get("/api/aktiviraj", (req: Request, res: Response) => this.aktivirajRacun(req, res));
        server.post("/api/ponovo-posalji-aktivaciju", (req: Request, res: Response) => this.ponovoPosaljiAktivaciju(req, res));
        
        // ===== KORISNIK INFO =====
        server.get("/api/korisnik", provjeriAutentikaciju, (req: Request, res: Response) => this.dajKorisnika(req, res));
    }
}
